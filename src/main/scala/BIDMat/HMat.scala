...
object HMat {
  ...
  def loadMat(fname: String, omat: Mat, compressed: Int): Mat = {
    ...
    gin.close
    ftype match {
      case 110 | 120 | 130 | 140 => loadFMat(fname, omat, compressed)
      case 201 | 202 | 231 | 241 | 301 | 302 | 331 | 341 => loadSMat(fname, compressed)
    }
  }
  ...
  def apply(dirname: String, filepat: String, varname: String, catd: Int): HMat = {
    ...
    files.foreach((fn: String) => {
      ...
      if (catd == 2) {
        ...
        blkinds(i + 1) = blkinds(i) + nc.toInt
        i += 1
      } else if (catd == 1) {
        if (ncols >= 0) {
          if (nc != 1) {
        ...
        blkinds(i + 1) = blkinds(i) + nr.toInt
        i += 1
      }
    })
    if (catd == 2) {
