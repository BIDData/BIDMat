...
case class FMat(dims0:Array[Int], val data:Array[Float]) extends DenseMat[Float](dims0, data) {
  ...
  override def reshapeTrim(newdims:Int*):FMat = reshapeTrim(newdims.toArray)
  
  override def reshapeTrim(newdims:Array[Int]):FMat = {
  ...
  override def reshapeTrim(adims:IMat):FMat = reshapeTrim(adims.data);
...
//  def GPUmult(b:FMat, out:Mat, btrans:Boolean) = GFunctions.GPUmult(this, b, out, btrans)
  ...
  def dot(a:FMat, omat:Mat):FMat = {
  	...
  	val out = FMat.newOrCheckFMat(odims, omat, GUID, a.GUID, "dot".##);
  ...
  def dotr(a:FMat, omat:Mat):FMat = {
   	...
   	val out = FMat.newOrCheckFMat(odims, omat, GUID, a.GUID, "dotr".##)
   	out.clear
   	if (!Mat.useMKL || length < 512) {
   		gdotr(a, out)
   	} else {
  ...
  override def blockGemm(transa:Int, transb:Int, nr:Int, nc:Int, k:Int, alpha:Float, aoff:Int, lda:Int, astep:Int,
      b:Mat, boff:Int, ldb:Int, bstep:Int, beta:Float, c:Mat, coff:Int, ldc:Int, cstep:Int, nreps:Int):FMat = {
  		blockGemm(transa, transb, nr, nc, k, alpha, aoff, lda, astep, b.asInstanceOf[FMat], boff, ldb, bstep,
  		    beta, c.asInstanceOf[FMat], coff, ldc, cstep, nreps);
  }
  
  def blockGemm4D(transa:Int, transb:Int, nr:Int, nc:Int, k:Int, alpha:Float, 
      aoff:Int, lda:Int, astep1:Int, astep2:Int,
      b:FMat, boff:Int, ldb:Int, bstep1:Int, bstep2:Int, beta:Float, 
      c:FMat, coff:Int, ldc:Int, cstep1:Int, cstep2:Int, nreps1:Int, nreps2:Int):FMat = {
    Mat.nflops += 2L * nr * nc * k * nreps1 * nreps2;
    blockSgemm4D(transa, transb, nr, nc, k, alpha, data, aoff, lda, astep1, astep2, 
  ...
  override def blockGemm4D(transa:Int, transb:Int, nr:Int, nc:Int, k:Int, alpha:Float, 
      ...
      c:Mat, coff:Int, ldc:Int, cstep1:Int, cstep2:Int, nreps1:Int, nreps2:Int):FMat = {
  		blockGemm4D(transa, transb, nr, nc, k, alpha, aoff, lda, astep1, astep2, b.asInstanceOf[FMat], boff, ldb, bstep1, bstep2,
  		    beta, c.asInstanceOf[FMat], coff, ldc, cstep1, cstep2, nreps1, nreps2);
  }
...
//		  case (gme:GMat, gkeys:GIMat) => gme.cumminByKey(gkeys, omat);
   ...
   */
  import GMat.BinOp._
  override def unary_-() = ffMatOpScalarv(-1f, FMat.vecMulFun, op_mul, null)
...
//  def *^! (b :FMat) = GPUmult(b, null, true)
   ...
   * Scalar operations
   */
  override def *  (b : Float) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null);
  override def +  (b : Float) = ffMatOpScalarv(b, FMat.vecAddFun, op_add, null)
  override def -  (b : Float) = ffMatOpScalarv(b, FMat.vecSubFun, op_sub, null)
  override def *@ (b : Float) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null)
  override def ∘  (b : Float) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null)
  override def /  (b : Float) = ffMatOpScalarv(b, FMat.vecDivFun, op_div, null)
  override def ^  (b : Float) = ffMatOpScalarv(b, FMat.vecPowFun, op_pow, null)

  override def >   (b : Float) = ffMatOpScalarv(b, FMat.vecGTFun, op_gt, null)
  override def <   (b : Float) = ffMatOpScalarv(b, FMat.vecLTFun, op_lt, null)
  override def ==  (b : Float) = ffMatOpScalarv(b, FMat.vecEQFun, op_eq, null)
  override def === (b : Float) = ffMatOpScalarv(b, FMat.vecEQFun, op_eq, null)
  override def >=  (b : Float) = ffMatOpScalarv(b, FMat.vecGEFun, op_ge, null)
  override def <=  (b : Float) = ffMatOpScalarv(b, FMat.vecLEFun, op_le, null)
  override def !=  (b : Float) = ffMatOpScalarv(b, FMat.vecNEFun, op_ne, null)
  
  override def min  (b : Float) = ffMatOpScalarv(b, FMat.vecMinFun, op_min, null)
  override def max  (b : Float) = ffMatOpScalarv(b, FMat.vecMaxFun, op_max, null)

  override def *  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, null);
  override def +  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecAddFun, op_add, null)
  override def -  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecSubFun, op_sub, null)
  override def *@ (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, null)
  override def ∘  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, null)
  override def /  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecDivFun, op_div, null)
  override def ^  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecPowFun, op_pow, null)
  override def >   (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecGTFun, op_gt, null)
  override def <   (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecLTFun, op_lt, null)
  override def ==  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecEQFun, op_eq, null)
  override def === (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecEQFun, op_eq, null)
  override def >=  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecGEFun, op_ge, null)
  override def <=  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecLEFun, op_le, null)
  override def !=  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecNEFun, op_ne, null)
  
  override def min  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecMinFun, op_min, null)
  override def max  (b : Double) = ffMatOpScalarv(b.toFloat, FMat.vecMaxFun, op_max, null)

  override def *  (b : Int) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null);
  override def +  (b : Int) = ffMatOpScalarv(b, FMat.vecAddFun, op_add, null)
  override def -  (b : Int) = ffMatOpScalarv(b, FMat.vecSubFun, op_sub, null)
  override def *@ (b : Int) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null)
  override def ∘  (b : Int) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null)
  override def /  (b : Int) = ffMatOpScalarv(b, FMat.vecDivFun, op_div, null)
  override def ^  (b : Int) = ffMatOpScalarv(b, FMat.vecPowFun, op_pow, null)
  override def >   (b : Int) = ffMatOpScalarv(b, FMat.vecGTFun, op_gt, null)
  override def <   (b : Int) = ffMatOpScalarv(b, FMat.vecLTFun, op_lt,  null)
  override def ==  (b : Int) = ffMatOpScalarv(b, FMat.vecEQFun, op_eq, null)
  override def === (b : Int) = ffMatOpScalarv(b, FMat.vecEQFun, op_eq, null)
  override def >=  (b : Int) = ffMatOpScalarv(b, FMat.vecGEFun, op_ge, null)
  override def <=  (b : Int) = ffMatOpScalarv(b, FMat.vecLEFun, op_le, null)
  override def !=  (b : Int) = ffMatOpScalarv(b, FMat.vecNEFun, op_ne, null)
  
  override def min  (b : Int) = ffMatOpScalarv(b.toFloat, FMat.vecMinFun, op_min, null)
  override def max  (b : Int) = ffMatOpScalarv(b.toFloat, FMat.vecMaxFun, op_max, null)
  
  override def *  (b : Long) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null);
  override def +  (b : Long) = ffMatOpScalarv(b, FMat.vecAddFun, op_add, null)
  override def -  (b : Long) = ffMatOpScalarv(b, FMat.vecSubFun, op_sub, null)
  override def *@ (b : Long) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null)
  override def ∘  (b : Long) = ffMatOpScalarv(b, FMat.vecMulFun, op_mul, null)
  override def /  (b : Long) = ffMatOpScalarv(b, FMat.vecDivFun, op_div, null)
  override def ^  (b : Long) = ffMatOpScalarv(b, FMat.vecPowFun, op_pow, null)
  override def >   (b : Long) = ffMatOpScalarv(b, FMat.vecGTFun, op_gt, null)
  override def <   (b : Long) = ffMatOpScalarv(b, FMat.vecLTFun, op_lt, null)
  override def ==  (b : Long) = ffMatOpScalarv(b, FMat.vecEQFun, op_eq, null)
  override def === (b : Long) = ffMatOpScalarv(b, FMat.vecEQFun, op_eq, null)
  override def >=  (b : Long) = ffMatOpScalarv(b, FMat.vecGEFun, op_ge, null)
  override def <=  (b : Long) = ffMatOpScalarv(b, FMat.vecLEFun, op_le, null)
  override def !=  (b : Long) = ffMatOpScalarv(b, FMat.vecNEFun, op_ne, null)
  
  override def min  (b : Long) = ffMatOpScalarv(b.toFloat, FMat.vecMinFun, op_min, null)
  override def max  (b : Long) = ffMatOpScalarv(b.toFloat, FMat.vecMaxFun, op_max, null)
...
//  def xTG (b :FMat) = mat.GPUmult(b, omat, true)
   ...
   */
  override def * (b : Float) = mat.ffMatOpScalarv(b, FMat.vecMulFun, op_mul, omat)
  override def + (b : Float) = mat.ffMatOpScalarv(b, FMat.vecAddFun, op_add, omat)
  override def - (b : Float) = mat.ffMatOpScalarv(b, FMat.vecSubFun, op_sub, omat)
  override def *@ (b : Float) = mat.ffMatOpScalarv(b, FMat.vecMulFun, op_mul, omat)
  override def ∘  (b : Float) = mat.ffMatOpScalarv(b, FMat.vecMulFun, op_mul, omat)
  override def /  (b : Float) = mat.ffMatOpScalarv(b, FMat.vecDivFun, op_div, omat)
  override def ^ (b : Float) = mat.ffMatOpScalar(b, FMat.powFun, op_pow, omat)

  override def > (b : Float) = mat.ffMatOpScalar(b, FMat.gtFun, op_gt, omat)
  override def < (b : Float) = mat.ffMatOpScalar(b, FMat.ltFun, op_lt, omat)
  override def == (b : Float) = mat.ffMatOpScalar(b, FMat.eqFun, op_eq, omat)
  override def >= (b : Float) = mat.ffMatOpScalar(b, FMat.geFun, op_ge, omat)
  override def <= (b : Float) = mat.ffMatOpScalar(b, FMat.leFun, op_le, omat)
  override def != (b : Float) = mat.ffMatOpScalar(b, FMat.neFun, op_ne, omat)
  
  override def max (b : Float) = mat.ffMatOpScalarv(b, FMat.vecMaxFun, op_max, omat)
  override def min (b : Float) = mat.ffMatOpScalarv(b, FMat.vecMinFun, op_min, omat)
  

  override def * (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, omat)
  override def + (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecAddFun, op_add, omat)
  override def - (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecSubFun, op_sub, omat)
  override def *@ (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, omat)
  override def ∘  (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, omat)
  override def /  (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecDivFun, op_div, omat)
  override def ^ (b : Double) = mat.ffMatOpScalar(b.toFloat, FMat.powFun, op_pow, omat)

  override def > (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecGTFun, op_gt, omat)
  override def < (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecLTFun, op_lt, omat)
  override def == (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecEQFun, op_eq, omat)
  override def >= (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecGEFun, op_ge, omat)
  override def <= (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecLEFun, op_le, omat)
  override def != (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecNEFun, op_ne, omat)
  
  override def max (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMaxFun, op_max, omat)
  override def min (b : Double) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMinFun, op_max, omat)

  override def * (b : Int) = mat.ffMatOpScalarv(b, FMat.vecMulFun, op_mul, omat)
  override def + (b : Int) = mat.ffMatOpScalarv(b, FMat.vecAddFun, op_add, omat)
  override def - (b : Int) = mat.ffMatOpScalarv(b, FMat.vecSubFun, op_sub, omat)
  override def *@ (b : Int) = mat.ffMatOpScalarv(b, FMat.vecMulFun, op_mul, omat)
  override def ∘  (b : Int) = mat.ffMatOpScalarv(b, FMat.vecMulFun, op_mul, omat)
  override def /  (b : Int) = mat.ffMatOpScalarv(b, FMat.vecDivFun, op_div, omat)
  override def ^ (b : Int) = mat.ffMatOpScalar(b, FMat.powFun, op_pow, omat)

  override def > (b : Int) = mat.ffMatOpScalarv(b, FMat.vecGTFun, op_gt, omat)
  override def < (b : Int) = mat.ffMatOpScalarv(b, FMat.vecLTFun, op_lt, omat)
  override def == (b : Int) = mat.ffMatOpScalarv(b, FMat.vecEQFun, op_eq, omat)
  override def >= (b : Int) = mat.ffMatOpScalarv(b, FMat.vecGEFun, op_ge, omat)
  override def <= (b : Int) = mat.ffMatOpScalarv(b, FMat.vecLEFun, op_le, omat)
  override def != (b : Int) = mat.ffMatOpScalarv(b, FMat.vecNEFun, op_ne, omat)
  
  override def max (b : Int) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMaxFun, op_max, omat)
  override def min (b : Int) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMinFun, op_min, omat)


  override def * (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, omat)
  override def + (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecAddFun, op_add, omat)
  override def - (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecSubFun, op_sub, omat)
  override def *@ (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, omat)
  override def ∘  (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMulFun, op_mul, omat)
  override def /  (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecDivFun, op_div, omat)
  override def ^ (b : Long) = mat.ffMatOpScalar(b.toFloat, FMat.powFun, op_pow, omat)

  override def > (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecGTFun, op_gt, omat)
  override def < (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecLTFun, op_lt, omat)
  override def == (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecEQFun, op_eq, omat)
  override def >= (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecGEFun, op_ge, omat)
  override def <= (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecLEFun, op_le, omat)
  override def != (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecNEFun, op_ne, omat)
  
  override def max (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMaxFun, op_max, omat)
  override def min (b : Long) = mat.ffMatOpScalarv(b.toFloat, FMat.vecMinFun, op_min, omat)
  /*
   * Specialize to IMat
   */
...
//      case gg:GIMat => gg.toFMat(out);
 ...
 def vecEQ(a:Array[Float], a0:Int, ainc:Int, b:Array[Float], b0:Int, binc:Int, c:Array[Float], c0:Int, cinc:Int, n:Int):Float = {
  ...
  def newOrCheckFMat(nr:Int, nc:Int, outmat:Mat, matGuid:Long, opHash:Int, forceCache:Boolean):FMat = {
  ...
  def newOrCheckFMat(nr:Int, nc:Int, outmat:Mat, matGuid:Long, opHash:Int):FMat =
      newOrCheckFMat(nr, nc, outmat, matGuid, opHash, false);
      
  def newOrCheckFMat(dims:Array[Int], out:Mat, matGuid:Long, opHash:Int, forceCache:Boolean):FMat = {
  ...
  def newOrCheckFMat(dims:Array[Int], outmat:Mat, matGuid:Long, opHash:Int):FMat =
      newOrCheckFMat(dims, outmat, matGuid, opHash, false);
  
  def newOrCheckFMat(dims:IMat, out:Mat, matGuid:Long, opHash:Int):FMat = 
    newOrCheckFMat(dims.data, out, matGuid, opHash, false);
  
  def newOrCheckFMat(dims:IMat, out:Mat, matGuid:Long, opHash:Int, forceCache:Boolean):FMat = 
    newOrCheckFMat(dims.data, out, matGuid, opHash, forceCache);


  def newOrCheckFMat(nr:Int, nc:Int, outmat:Mat, guid1:Long, guid2:Long, opHash:Int, forceCache:Boolean):FMat = {
    if (outmat.asInstanceOf[AnyRef] != null || (!Mat.useCache && !forceCache)) {
      newOrCheckFMat(nr, nc, outmat)
    } else {
  ...
  def newOrCheckFMat(nr:Int, nc:Int, outmat:Mat,guid1:Long, guid2:Long, opHash:Int):FMat =
      newOrCheckFMat(nr, nc, outmat, guid1, guid2, opHash, false);
  
   def newOrCheckFMat(dims:Array[Int], out:Mat, guid1:Long, guid2:Long, opHash:Int, forceCache:Boolean):FMat = {
    if (out.asInstanceOf[AnyRef] != null || (!Mat.useCache && !forceCache)) {
      newOrCheckFMat(dims, out)
    } else {
      val key = (guid1, guid2, opHash)
  ...
  def newOrCheckFMat(dims:Array[Int], outmat:Mat,guid1:Long, guid2:Long, opHash:Int):FMat =
      newOrCheckFMat(dims, outmat, guid1, guid2, opHash, false);
  
  def newOrCheckFMat(dims:IMat, out:Mat, guid1:Long, guid2:Long, opHash:Int):FMat = 
    newOrCheckFMat(dims.data, out, guid1, guid2, opHash, false);
  
  def newOrCheckFMat(dims:IMat, out:Mat, guid1:Long, guid2:Long, opHash:Int, forceCache:Boolean):FMat = 
    newOrCheckFMat(dims.data, out, guid1, guid2, opHash, forceCache);

  def newOrCheckFMat(nr:Int, nc:Int, outmat:Mat, guid1:Long, guid2:Long, guid3:Long, opHash:Int, forceCache:Boolean):FMat = {
    if (outmat.asInstanceOf[AnyRef] != null || (!Mat.useCache && !forceCache)) {
      newOrCheckFMat(nr, nc, outmat)
    } else {
  ...
  def newOrCheckFMat(nr:Int, nc:Int, outmat:Mat,guid1:Long, guid2:Long, guid3:Long, opHash:Int):FMat =
      newOrCheckFMat(nr, nc, outmat, guid1, guid2, guid3, opHash, false);
  
  def newOrCheckFMat(dims:Array[Int], out:Mat, guid1:Long, guid2:Long, guid3:Long, opHash:Int, forceCache:Boolean):FMat = {
    if (out.asInstanceOf[AnyRef] != null || (!Mat.useCache && !forceCache)) {
      newOrCheckFMat(dims, out)
    } else {
      val key = (guid1, guid2, guid3, opHash)
  ...
  def newOrCheckFMat(dims:Array[Int], outmat:Mat,guid1:Long, guid2:Long, guid3:Long, opHash:Int):FMat =
      newOrCheckFMat(dims, outmat, guid1, guid2, guid3, opHash, false);
  
  def newOrCheckFMat(dims:IMat, out:Mat, guid1:Long, guid2:Long, guid3:Long, opHash:Int):FMat = 
    newOrCheckFMat(dims.data, out, guid1, guid2, guid3, opHash, false);
  
   def newOrCheckFMat(dims:IMat, out:Mat, guid1:Long, guid2:Long, guid3:Long, opHash:Int, forceCache:Boolean):FMat = 
    newOrCheckFMat(dims.data, out, guid1, guid2, guid3, opHash, forceCache);
