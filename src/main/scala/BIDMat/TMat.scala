    ...
    while (i < y.length) {
     tiles(i) match {
       case denseMat : DenseMat[T] =>          
                ...
                out.update(rowInds,colInds,denseMat)
                i += 1
        }
    }
    out
...
def tDMult(a:DenseMat[T], outmat:Mat) : DenseMat[T] =  {
		...
		var i = 0
  		...
  		var out = FMat.newOrCheckFMat(nrows, a.ncols, outmat, GUID, a.GUID, "tMult".##)
  		  ...
  		  Mat.nflops += 2L * m.length * a.ncols
		       ...
		       m match {
                       case fMat : FMat =>
                            fMat.tileMult(fMat.nrows, a.ncols, fMat.ncols, 0, 0, a, x(i), 0, tmp, y(i), 0)
                            tmp.ggMatOp(out,Mop_Plus, out)
                       }
