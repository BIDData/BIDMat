...
class FFilter(inDims0:IMat, outDims0:IMat, stride0:IMat, pad0:IMat, data0:Array[Float]) extends
	...
	if (idim > 0) {
		...
		while (j < outDims(0)) {                    // Move over output tensor
		    val jfwidth = j * fwidth;
		    var ss = 0f;
		    var i = 0;
		    while (i < fwidth) {                      // Move over input tensor
			ss += in.data(astart + k + fwidth - i - 1) * data0(fstart + jfwidth + i);
			i += 1;
		    }
	...
	val inpadded = if (pad.asInstanceOf[AnyRef] != null && SciFunctions.sum(pad).dv > 0) pad(a, pad) else a;
	out.clear;
	_correlate(inpadded, out, inDims.length-1, 0, 0, 0);
	out
    };
	...
	if (idim > 0) {
		...
		while (j < outDims(0)) {                    // Move over output tensor
		    ...
		    var i = 0;
		    while (i < fwidth) {                      // Move over input tensor
			ss += in.data(astart + k + i) * data0(fstart + jfwidth + i);
			i += 1;
		    }
