...
case class A4D(val dim1:Int, val dim2:Int, val dim3:Int, val dim4:Int, val data:Array[Float]) { 
  ...
  def apply(indx:Int):Float = { 
    if (indx < length) { 
      ...
      throw new RuntimeException("A4D index out of range")
    }
  }
  ...
  def apply(x:Int, y:Int, z:Int, w:Int):Float = { 
    ...
    if (indx < length) { 
      ...
      throw new RuntimeException("A4D indices out of range")
    }
  }
  ...
  def rotate(todo:Int, perm:IMat, dims:IMat) = { 
    ...
    if (i > todo) { 
      throw new RuntimeException("A4D.permute: bad permutation");
    }
  }

  def permute(p1:Int, p2:Int, p3:Int, p4:Int):A4D = permute(irow(p1,p2,p3,p4))

  def permute(perm:IMat):A4D = { 
    if (perm.length != 4) { 
      throw new RuntimeException("A4D permute bad permutation ")
    }
    val dims = irow(dim1, dim2, dim3, dim4)
    val iperm = invperm(perm)
    var out = A4D(dims(iperm(0)), dims(iperm(1)), dims(iperm(2)), dims(iperm(3)))
    var out2 = A4D(dims(iperm(0)), dims(iperm(1)), dims(iperm(2)), dims(iperm(3)))
...
object A4D {
  ...
  def apply(dim1:Int, dim2:Int, dim3:Int, dim4:Int) = new A4D(dim1, dim2, dim3, dim4, new Array[Float](dim1*dim2*dim3*dim4))

  def apply(dim1:Int, dim2:Int, dim3:Int, dim4:Int, f:FMat):A4D = { 
    if (dim1*dim2*dim3*dim4 == f.length) { 
      new A4D(dim1, dim2, dim3, dim4, f.data)
    } else { 
