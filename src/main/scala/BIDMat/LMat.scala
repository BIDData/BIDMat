...
case class LMat(nr:Int, nc:Int, data0:Array[Long]) extends DenseMat[Long](nr, nc, data0) { 
  ...
  override def t:LMat = LMat(gt(null))
...
//  	  case aa:GIMat => aa.copyFrom(this)
  ...
  def * (b : Float) = iMult(LMat.lelem(b.toLong), null)
  def + (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecAddFun, null)
  def - (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecSubFun, null)
  def *@ (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecMulFun, null)
  def ∘  (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecMulFun, null)
...
//  def ^ (b : Int) = mat.iiMatOpScalar(b, (x:Float, y:Float) => math.pow(x,y).toFloat, null)
  ...
  def > (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecGTFun, null)
  def < (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecLTFun, null)
  def == (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecEQFun, null)
  def >= (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecGEFun, null)
  def <= (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecLEFun, null)
  def != (b : Float) = iiMatOpScalarv(b.toLong, LMat.vecNEFun, null)
...
class LPair(val omat:Mat, val mat:LMat) extends Pair {
  ...
  override def t:LMat = LMat(mat.gt(omat))
...
//  override def ^ (b : Long) = mat.iiMatOpScalar(b, (x:Float, y:Float) => math.pow(x,y).toFloat, omat)
  ...
  def * (b : Int) = mat.iMult(LMat.lelem(b), omat)
  def + (b : Int) = mat.iiMatOpScalarv(b, LMat.vecAddFun, omat)
  def - (b : Int) = mat.iiMatOpScalarv(b, LMat.vecSubFun, omat)
  def *@ (b : Int) = mat.iiMatOpScalarv(b, LMat.vecMulFun, omat)
  def ∘  (b : Int) = mat.iiMatOpScalarv(b, LMat.vecMulFun, omat)
  def / (b : Int) = mat.iiMatOpScalarv(b, LMat.vecDivFun, omat)
...
//  override def ^ (b : Int) = mat.iiMatOpScalar(b, (x:Float, y:Float) => math.pow(x,y).toFloat, omat)
  ...
  def > (b : Int) = mat.iiMatOpScalarv(b, LMat.vecGTFun, omat)
  def < (b : Int) = mat.iiMatOpScalarv(b, LMat.vecLTFun, omat)
  def == (b : Int) = mat.iiMatOpScalarv(b, LMat.vecEQFun, omat)
  def >= (b : Int) = mat.iiMatOpScalarv(b, LMat.vecGEFun, omat)
  def <= (b : Int) = mat.iiMatOpScalarv(b, LMat.vecLEFun, omat)
  def != (b : Int) = mat.iiMatOpScalarv(b, LMat.vecNEFun, omat) 
