...
class GDMat(nr:Int, nc:Int, var data:Pointer, val realsize:Int) extends Mat(nr, nc) {
  ...
  def update(i:Int, j:Int, v:Double):GDMat = {
    val tmp = new Array[Double](1)
    tmp(0) = v
...
//      }
  ...
  def copyTo(a:GIMat):GIMat = {
    if (nrows != a.nrows || ncols != a.ncols)
      throw new RuntimeException("dimensions mismatch in GIMat <-- GDMat")
...
//    if (data != null) free
  ...
  def *@ (a : Int) = gOp(GDMat(a.toFloat), null, op_mul)
  def ∘  (a : Int) = gOp(GDMat(a.toFloat), null, op_mul)
  def /  (a : Int) = gOp(GDMat(a.toFloat), null, op_div)
  def ^  (a : Int) = gOp(GDMat(a.toFloat), null, op_pow)
